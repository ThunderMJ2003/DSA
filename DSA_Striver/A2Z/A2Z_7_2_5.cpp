...
class Solution {
    ...
    void fWhelper(int &fin, vector<int> nums, int k, int cursum){
        if (nums.empty()){
            if (cursum == k){
                ++fin;
            }
        ...
        fWhelper(fin, nums, k, cursum+x);
        fWhelper(fin, nums, k, cursum);
    }
    int findWays(vector<int>& arr, int k){
        int fin = 0;
        fWhelper(fin, arr, k, 0);
        return fin;
    }
};
